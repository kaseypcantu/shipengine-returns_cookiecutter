...
class ShippingAddressForm(FlaskForm):
    ...
    address_line_2 = StringField(
        "Address Line 2", validators=[Optional(), Length(max=60)]
    )
    address_line_3 = StringField(
        "Address Line 3", validators=[Optional(), Length(max=60)]
    )
...
class SchedulePickupForm(FlaskForm):
    ...
    pickup_window_start_at = DateTimeLocalField(
        "Pickup Window: Start - Begin at 9 AM",
        validators=[InputRequired(), DataRequired()],
    )
    pickup_window_end_at = DateTimeLocalField(
        "Pickup Window: End - Ends at 5 PM",
        validators=[InputRequired(), DataRequired()],
    )
    pickup_notes = TextAreaField("Pickup Notes", validators=[Optional()])
