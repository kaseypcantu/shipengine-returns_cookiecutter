...
from flask_server_funko_returns.main_routes.forms import ShippingAddressForm, SchedulePickupForm
from flask_server_funko_returns.models import ShipToAddress, ShipFromAddress, Package, PackageWeight, PackageDimensions
...
def home():
    ...
    if request.method == "GET":
        return render_template("returns_portal.html", title="flask server funko Returns Portal", form=form)
    ...
    if status_code[0] == "5":
        print(se_data)
        return render_template("errors/se-5XX.html", title="flask server funko Returns Portal - SE Error", error_data=json.dumps(se_data, indent=2, separators=(",", ": ")), error_status=status_code)
    elif status_code[0] == "4":
        print(se_data)
        return render_template("errors/se-4XX.html", title="flask server funko Returns Portal - SE Error", error_data=json.dumps(se_data, indent=2, separators=(",", ": ")), error_status=status_code)
