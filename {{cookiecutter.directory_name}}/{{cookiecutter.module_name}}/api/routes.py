...
def schedule_pickup():
    ...
    if int(str(status_code)[0]) == 5:
        print(se_pickup)
        return render_template("errors/se-5XX.html", title="{{cookiecutter.project_name}} - SE Error",
                               error_data=se_pickup)
    elif int(str(status_code)[0]) == 4:
        print(se_pickup)
        return render_template("errors/se-4XX.html", title="{{cookiecutter.project_name}} - SE Error",
                               error_data=se_pickup)
